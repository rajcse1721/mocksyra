...
test('authenticated directed workflow supports restore, collaboration, leaving, guarded finishing, mutual feedback and cancellation', { timeout: 30_000 }, async t => {
  ...
  const preferences = { name: 'Ada', languages: ['JavaScript'], slots: [new Date(Date.now() + 4 * 60 * 60 * 1000).toISOString()], interviewType: 'Frontend', experience: 'Intermediate', spokenLanguage: 'English', timezone: 'UTC' };

  assert.equal((await interviewer.ack('find-match', { ...preferences, name: 'Sam', practiceMode: 'interviewer' })).waiting, true);
  const found = await candidate.ack('find-match', { ...preferences, practiceMode: 'candidate' });
