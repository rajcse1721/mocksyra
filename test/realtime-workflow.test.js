...
test('authenticated directed workflow supports restore, collaboration, leaving, guarded finishing, mutual feedback and cancellation', { timeout: 30_000 }, async t => {
  ...
  const clients = [];
  let child;
  const identities = { candidate: 'candidate@example.test', interviewer: 'interviewer@example.test', outsider: 'outsider@example.test', newcomer: 'newcomer@example.test' };
  ...
  const directBooking = await newcomer.ack('book-listing', { listingId: competingCandidate.profile.listingId, slot: preferences.slots[0], profile: { name: 'New User', experience: 'Beginner', languages: ['JavaScript'], spokenLanguage: 'English', timezone: 'UTC' } });
  ...
  const offeredListing = available.listings.find(item => item.listingId === offered.profile.listingId);
  ...
  const found = await candidate.ack('book-listing', { listingId: offered.profile.listingId, slot: offeredListing.slots[0] });
  ...
  assert.equal((await outsider.ack('book-listing', { listingId: offered.profile.listingId, slot: offeredListing.slots[0] })).ok, false, 'a listed session cannot be double booked');
