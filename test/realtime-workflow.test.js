...
test('authenticated directed workflow supports restore, collaboration, leaving, guarded finishing, mutual feedback and cancellation', { timeout: 30_000 }, async t => {
  ...
  const busyInterviewerClient = await connectSocket(baseUrl, 'busyPeerOne'); clients.push(busyInterviewerClient);
  assert.equal((await busyInterviewerClient.ack('set-meeting-link', { roomId: 'busy-live-room', meetingUrl: 'https://zoom.us/j/123456789' })).ok, false, 'an in-progress legacy call cannot switch providers and leave media capture behind');
  ...
  assert.equal(match.videoProvider, 'external');
  assert.equal(match.meetingUrl, null);
  ...
  assert.equal((await candidate.ack('set-meeting-link', { roomId, meetingUrl: 'https://zoom.us/j/123456789' })).ok, false, 'a candidate cannot manage the call link');
  assert.equal((await outsider.ack('set-meeting-link', { roomId, meetingUrl: 'https://zoom.us/j/123456789' })).ok, false, 'an outsider cannot manage the call link');
  assert.equal((await interviewer.ack('set-meeting-link', { roomId, meetingUrl: 'https://zoom.us.evil.test/j/123456789' })).ok, false, 'deceptive meeting hosts are rejected');
  const linkUpdate = candidate.next('meeting-link-updated');
  const savedLink = await interviewer.ack('set-meeting-link', { roomId, meetingUrl: 'https://teams.live.com/meet/123456789' });
  assert.equal(savedLink.ok, true);
  assert.equal(savedLink.match.meetingProvider, 'teams');
  assert.equal(savedLink.match.meetingUrl, 'https://teams.live.com/meet/123456789');
  assert.equal((await linkUpdate).meetingUrl, savedLink.match.meetingUrl, 'the other participant receives the private meeting link update');
  const preparedCall = await candidate.ack('prepare-call', { roomId });
  assert.equal(preparedCall.provider, 'external');
  assert.equal(preparedCall.meetingUrl, savedLink.match.meetingUrl);
  ...
  assert.equal(restored.activeMatch.meetingUrl, savedLink.match.meetingUrl, 'the meeting link survives an authenticated restore');
  assert.equal(restored.notifications.filter(item => item.matchId === roomId && item.title === 'Your interview link is ready').length, 1);
  const replacementUpdate = candidate.next('meeting-link-updated');
  const replacement = await interviewer.ack('set-meeting-link', { roomId, meetingUrl: 'https://teams.live.com/meet/987654321' });
  assert.equal(replacement.ok, true);
  assert.equal((await replacementUpdate).meetingUrl, replacement.match.meetingUrl);
  candidate.emit('restore-profile');
  const afterReplacement = await candidate.next('profile-state');
  assert.equal(afterReplacement.activeMatch.meetingUrl, replacement.match.meetingUrl);
  assert.equal(afterReplacement.notifications.filter(item => item.matchId === roomId && item.title === 'Your interview link is ready').length, 1, 'editing a link does not spam the candidate with more notifications or email');
